...
make_gridpack () {
    ...
    MGSOURCE=https://jixiao.web.cern.ch/jixiao/genval/mg5amcnlo.tar.gz
    ...
    # [NLO_Gridpack] nevents should be 0 to generate run.sh in NLO mode
    if [ "$isnlo" -gt "0" ]; then
      sed -i "s/^.*=\s*nevents.*$/  0     = nevents ! Number of unweighted events requested/" ./Cards/run_card.dat
    fi
    ...
    if [ "$isnlo" -gt "0" ]; then
        ...
        if [ -e $CARDSDIR/${name}_madspin_card.dat ]; then
            ...
            # [NLO_Gridpack] set the ms_dir to ../../process/madspin
            sed -i "s/^\s*set\s*ms_dir.*$/set ms_dir ..\/..\/process\/madspin/" ./Cards/madspin_card.dat
            mv ./Cards/madspin_card.dat ./Cards/tmp_madspin_card
        fi

        # [NLO_Gridpack] launch the process firstly
        cd $WORKDIR
        mv processtmp process # move processtmp to process
        cd $WORKDIR/process
        # echo "launch ${WORKDIR}/process"
        echo "shower=OFF" > makegrid.dat
        echo "reweight=OFF" >> makegrid.dat
        # [NLO_Gridpack] madspin setting for NLO gridpack
        ...
        # [NLO_Gridpack] use bin/mg5_aMC rather than bin/generate_events
        cat makegrid.dat | ./bin/generate_events -n run_01
        ...
        echo "finished run_01 run"
        echo "[ runcmsgrid.sh ] switch to ReadOnly mode"
        chmod -R 555 $WORKDIR/process

        # [NLO_Gridpack] run the gridpack in ReadOnly mode to generate a sample to setup madspin
        echo "[ runcmsgrid.sh ] run the gridpack in RO mode to generate a sample to setup madspin"
        mkdir -p $WORKDIR/tmp_gridpack/forMS
        cd $WORKDIR/tmp_gridpack/forMS
        $WORKDIR/process/bin/run.sh 1000 123
        # [NLO_Gridpack] remove ReadOnly and setup madspin within the gridpack, the madspin card need to use ms_dir option
        echo "[ runcmsgrid.sh ] remove ReadOnly and setup madspin within the gridpack"
        echo "[ runcmsgrid.sh ] note that the madspin card need to use ms_dir option"
        chmod -R 777 $WORKDIR/process
        cd $WORKDIR/tmp_gridpack/forMS
        #when MADSPIN=OFF is applied, mg5_aMC moves madspin_card.dat to .madspin_card.dat
        if [ -e $WORKDIR/process/Cards/tmp_madspin_card ]; then
            sed -i '1s/^/import .\/events.lhe.gz\n/' $WORKDIR/process/Cards/tmp_madspin_card
            $WORKDIR/$MGBASEDIRORIG/MadSpin/madspin < $WORKDIR/process/Cards/tmp_madspin_card
        fi
        cd $WORKDIR/process
        # rm -rf $WORKDIR/tmp_gridpack # for debug
        #
        if [ -e $CARDSDIR/${name}_externaltarball.dat ]; then
    ...
    # [NLO_Gridpack] generate the pointer file
    if [ "$isnlo" -gt "0" ]; then
        cp $PRODHOME/mk_workdir.py .
        chmod 777 runcmsgrid.sh
        tar -zcvf ${PRODHOME}/${name}_${scram_arch}_${cmssw_version}_pointer.gz runcmsgrid.sh mk_workdir.py
        echo "Pointer file for the NLO ReadOnly gridpack is created successfully at ${PRODHOME}/${name}_${scram_arch}_${cmssw_version}_pointer.tar.xz"
    fi
}
